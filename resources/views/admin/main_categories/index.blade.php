...
@section('sub_title')
    ...
    <a href="{{ url('admin_panel/main_categories/create') }}" class="btn btn-light-warning font-weight-bolder btn-sm">
        اضافه قسم جديد
    </a>
            ...
            <table class="table table-bordered table-checkable" id="m_table_1">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <img src="{{ Custom_Image_Path('main_categories_images',$value->pic) }}" style="width: 130px;height:100px;display: block;margin: auto;">
                         </td>

                        <td>
                            <a href="{{ url('admin_panel/main_categories/'). '/' . $value->id . '/edit'}}" class='m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill' title='Edit'>
        ...
        $(document).ready(function () {
            ...
            $('#m_table_1').on('click', '.un_active', function () {
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            window.location.href = '{{ url('admin_panel/main_categories/un_active') }}' + '/' + ID;
            ...
            $('#m_table_1').on('click', '.active', function () {
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            window.location.href = '{{ url('admin_panel/main_categories/active') }}' + '/' + ID;
            ...
            $(".order_input").on('change', function() {
                ...
                $.ajax({
                    url: '{{ url('admin_panel/main_categories/update-order') }}',
                    method: "post",
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: ele.attr("data-id"),
